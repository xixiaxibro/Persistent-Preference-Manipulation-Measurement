...
OUTPUT_DIR=${OUTPUT_DIR:-$RUN_ROOT/03b_target_analysis}
LOG_FILE=${LOG_FILE:-$OUTPUT_DIR/analyze_target_urls.log}
PID_FILE=${PID_FILE:-$OUTPUT_DIR/analyze_target_urls.pid}
TOP_N=${TOP_N:-200}
...
if [[ -e "$LOG_FILE" || -e "$PID_FILE" ]]; then
  echo "ERROR: target analysis log or pid file already exists under: $OUTPUT_DIR" >&2
...
CMD=(
  python3 -u analyze_target_urls.py
  --input "$INPUT_FILE"
